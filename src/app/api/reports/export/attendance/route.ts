// Export Attendance Report API Route
...
export async function POST(request: Request) {
    try {
        ...
        report.forEach((batch) => {
            batch.students.forEach((student) => {
                flatData.push({
                    ...
                    'Attendance Percentage': `${student.attendancePercentage.toFixed(2)}%`,
                });
            });
        });
        ...
        switch (format) {
            case 'CSV':
                content = await exportService.exportAttendanceToCSV(flatData);
                contentType = 'text/csv';
                filename = 'attendance-report.csv';
                break;
            case 'PDF':
                content = await exportService.exportAttendanceToPDF(flatData);
                contentType = 'application/pdf';
                filename = 'attendance-report.pdf';
                break;
            case 'EXCEL':
                content = await exportService.exportAttendanceToExcel(flatData);
